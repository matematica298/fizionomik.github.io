...
const vk = new VK();
$confirmation_token = '2e3b9126';
const {updates} = vk; //////////////////////////////////// BOT BY BOTSSOFT
const fs = require("fs"); /////////////////////////////////////////////////////////////////////
const admins = [443389649]; ////// id админа
const vip = [443389649]; /////////// id админа
...
vk.setOptions({
    token: '8c9bd7643d0ff4ae75cc0f30db3e002a20c13a113f588d2b0d9ac2f52db2a5b16a0636f246c1c24005b76', // токен группы
    apiMode: 'parallel',
	pollingGroupId: 184626603 // 1 замени на id группы
});
...
vk.updates.use(async (message, next) => {  
	...
	vk.updates.hear(/^(?:бот)$/i, (message) => {
		...
		return message.send(`
			💟 ➾ Проект: ${config.bot}
			💻 ➾ Версия: 1.5
			🌈 ➾ Создатель: Fizionomik

			🍷 ➾ Зарегистрировано: ${acc.number}
			☀ ➾ Казна штата Gleger:  ${acc.kazna} 💰
            💬 ➾ Сообщений:  ${acc.msg}
    ...
    if (message.is("message") && message.isOutbox)
		...
		message.send(`👴 *id${message.user}, Привет, ты зарегестрировался
		,Напишите помощь`)
...
🎁 Для Вас доступны подарки ! Чтобы открыть, напиши " подарок "
 ...
 vk.updates.hear(/^(?:элитмагаз)$/i,  (message) => { 
    ...
    💻 Покупка осуществляется за реальные деньги. По поводу покупки, пишите главному администратору: https://vk.com/fixsick
...
//\\\\\\\\\\\ РАБОТЫ \\\\\\\\\\\\\\\\\\\\\\\\\\\\\\
 ...
 vk.updates.hear(/^(?:работы)\s?([1-9]+)?/i, message => {
  ...
  vk.updates.hear(/^(?:работать)/i, message => {
	...
	if(acc.kazna < 999) return message.send(`🙅‍♂ Простите, в казне штата Gliger закончились деньги ! Попробуй позже!`);
	if(user.job.stop != false) return message.send(`👨‍⚖️ >> Работать можно раз в час. Отдохните!`);
 	if(user.job.stop != false) return message.send(`👨‍⚖️ >> Работать можно раз в час. Отдохните!`);
